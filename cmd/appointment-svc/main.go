...
import (
	...
	"github.com/Sotiris-Bekiaris/careflow-mini/internal/appointment"
...
func main() {
	...
	tracerProvider, err := observability.InitTracer("appointment-svc", getEnv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4318"))
	if err != nil {
	...
	tracer := observability.GetTracer("appointment-svc")

	// Initialize metrics
	_, err = observability.InitMetrics("appointment-svc")
	if err != nil {
	...
	repo := appointment.NewPostgresRepository(dbPool)
	svc := appointment.NewService(repo, publisher, tracer)
	handler := appointment.NewHandler(svc)
	...
	// Register services
	appointmentv1.RegisterAppointmentServiceServer(grpcServer, handler)
