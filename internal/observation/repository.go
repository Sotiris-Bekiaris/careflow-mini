...
type Repository interface {
	Create(ctx context.Context, observation *fhir.Observation) (*fhir.Observation, error)
	GetByID(ctx context.Context, id string) (*fhir.Observation, error)
	Update(ctx context.Context, observation *fhir.Observation) (*fhir.Observation, error)
	ListByPatient(ctx context.Context, patientID string, limit int32, offset int32) ([]*fhir.Observation, string, error)
...
func (r *PostgresRepository) Create(ctx context.Context, observation *fhir.Observation) (*fhir.Observation, error) {
	if observation.ID == "" {
		observation.ID = uuid.New().String()
	}

	if observation.Meta.LastUpdated.IsZero() {
		observation.Meta.LastUpdated = time.Now().UTC()
	}

	// Extract patient ID from subject reference
	patientID := ""
	if observation.Subject.Reference != "" && len(observation.Subject.Reference) > 8 {
		patientID = observation.Subject.Reference[8:] // Remove "Patient/" prefix
	}
	...
	query := `
		INSERT INTO lab_svc.observations (id, fhir_resource, status, patient_id, effective_datetime, created_at, updated_at)
...
func (r *PostgresRepository) GetByID(ctx context.Context, id string) (*fhir.Observation, error) {
	query := `
...
func (r *PostgresRepository) Update(ctx context.Context, observation *fhir.Observation) (*fhir.Observation, error) {
	if observation.ID == "" {
		return nil, errors.New("observation ID is required for update")
	}
	...
	query := `
		UPDATE lab_svc.observations
		SET fhir_resource = $2, status = $3, patient_id = $4, effective_datetime = $5, updated_at = $6
		WHERE id = $1
		RETURNING fhir_resource
	`
	...
	var updatedObservation fhir.Observation
	if err := json.Unmarshal(result, &updatedObservation); err != nil {
		return nil, fmt.Errorf("failed to unmarshal result: %w", err)
	}
...
// ListByPatient lists observations for a patient with pagination
func (r *PostgresRepository) ListByPatient(ctx context.Context, patientID string, limit int32, offset int32) ([]*fhir.Observation, string, error) {
	if limit == 0 {
		limit = 10
	}
	if limit > 100 {
...
func (r *PostgresRepository) Delete(ctx context.Context, id string) error {
	query := `
		DELETE FROM lab_svc.observations
		WHERE id = $1
	`
